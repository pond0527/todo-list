...
export const STATUS_LIST_FILEPATH = join(process.cwd(), 'share/status-list.json');
console.log(STATUS_LIST_FILEPATH);

const handler = async (req: NextApiRequest, res: NextApiResponse<ApiResoinse<Status[]>>) => {
    if (req.method === 'GET') {
        const statusList = await getStatusList();
        console.log("Get: statusList, ",statusList);
